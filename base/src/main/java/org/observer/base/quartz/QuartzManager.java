package org.observer.base.quartz;
...
public class QuartzManager {
    ...
    public void addJob(String jobName, int taskId, String jobGroupName, Class clazz, String[] fun, Map<String, String> dataMap, SimpleScheduleBuilder builder) {
        JobDetail jobDetail = JobBuilder.newJob(clazz)
                .withIdentity(jobName,jobGroupName).build();

        jobDetail.getJobDataMap().put(Plugin.PARAM,dataMap);
        jobDetail.getJobDataMap().put(Plugin.FUNCTIONS, Arrays.asList(fun));
        jobDetail.getJobDataMap().put("taskId", taskId);
        jobDetail.getJobDataMap().put("groupName", jobGroupName);


        Trigger trigger = TriggerBuilder.newTrigger().withIdentity(jobName,jobGroupName).startNow().withSchedule(builder).build();
        // 交由Scheduler安排触发
        try {
